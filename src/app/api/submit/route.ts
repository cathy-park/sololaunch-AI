...
export async function POST(req: Request) {
  try {
    ...
    await resend.emails.send({
      from: '똑디 인터뷰 <onboarding@resend.dev>',
      to: adminEmail,
      subject: `[똑디] ${client_name}님의 고도화 견적 및 상담 브리프가 접수되었습니다.`,
      html: `
        <div style="font-family: 'Pretendard', sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #000; padding: 50px; color: #000; background: #fff;">
          ...
          <div style="text-align: center;">
            <a href="${req.headers.get('origin')}/admin/${dbData.id}" style="display: inline-block; padding: 18px 45px; background: #000; color: #fff; text-decoration: none; font-weight: 900; font-size: 12px; letter-spacing: 0.1em;">어드민에서 상세 확인하기</a>
          </div>
        </div>
      `,
    });
